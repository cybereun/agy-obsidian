...
import type { AgyObsidianSettings, PermissionMode, PreferredModel } from '../types';
...
export async function runExternalCli(input: ExternalCliRunInput): Promise<ExternalCliRunResult> {
  ...
  if (!command) {
    throw new Error(`${input.id} CLI was not detected. Open AgyObsidian settings and run Auto-detect all.`);
  }
...
function buildExternalPrompt(input: ExternalCliRunInput): string {
  const parts = [
    'You are being called from the AgyObsidian Obsidian plugin as a collaborating local CLI agent.',
...
export function getExternalCliRunDirectory(): string {
  const dir = path.join(os.tmpdir(), 'agy-obsidian-cli-runs');
