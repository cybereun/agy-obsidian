...
import {
  ...
  AGY_OBSIDIAN_CLAUDE_PLUGIN_NAME,
...
export function getAntigravityAuthPreview(agyPath = 'agy'): string {
  return `${agyPath} --print-timeout 10m --print "Check Google Sign-In for AgyObsidian."`;
}
...
export async function startGoogleSignIn(agyPath: string, envText: string, cwd: string, log: InstallLog): Promise<void> {
  ...
  const args = [
    ...
    '--print',
    [
      'Check Google Sign-In for AgyObsidian.',
...
export async function convertClaudeToolsForAntigravity(
  ...
  log(`\n$ ${detected} plugin enable ${AGY_OBSIDIAN_CLAUDE_PLUGIN_NAME}\n`);
  await runProcessBestEffort(
    detected,
    ['plugin', 'enable', AGY_OBSIDIAN_CLAUDE_PLUGIN_NAME],
    env,
    cwd,
    log,
