...
export async function generateVisualAsset(request: GenerateVisualAssetRequest): Promise<GeneratedVisualAsset> {
  ...
  const folder = request.mediaFolder.trim() || 'attachments/agy-obsidian';
...
export async function draftVisualPrompt(request: GenerateVisualAssetRequest): Promise<string> {
  ...
  for await (const event of request.agent.query({
    ...
    if (event.type === 'error') {
      ...
      console.warn('[AgyObsidian visual] Prompt draft warning:', event.content);
    }
  }
...
function buildAntigravityImageGenerationPrompt(vaultRelativePath: string, visualPrompt: string): string {
  return [
    ...
    '- Do not generate video or TTS; those are intentionally capability-gated for future AgyObsidian versions.',
    ...
    '- Do not modify the source note. AgyObsidian will embed the image after the file exists.',
    '',
    'AgyObsidian workflow context:',
