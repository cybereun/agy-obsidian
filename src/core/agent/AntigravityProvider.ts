...
import type { AgentEvent, AgentProvider, AgentQuery, AgyObsidianSettings } from '../types';

export class AntigravityProvider implements AgentProvider {
  private settings: () => AgyObsidianSettings;
  private currentProcess: ChildProcess | null = null;
  private sessionId: string | null = null;

  constructor(settings: () => AgyObsidianSettings) {
  ...
  private buildPrompt(input: AgentQuery): string {
    const parts: string[] = [];
    parts.push('You are Antigravity CLI running from an Obsidian plugin workflow named AgyObsidian.');
