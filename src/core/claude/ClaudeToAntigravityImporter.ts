...
export const AGY_OBSIDIAN_CLAUDE_PLUGIN_NAME = 'agy-obsidian-claude-tools';
...
export function convertClaudeToolsToAntigravityPlugin(log?: InstallLog): ClaudeToAntigravityImportResult {
  const pluginDir = path.join(os.homedir(), '.gemini', 'agy-obsidian', 'plugins', AGY_OBSIDIAN_CLAUDE_PLUGIN_NAME);
  ...
  const pluginJson = {
    name: AGY_OBSIDIAN_CLAUDE_PLUGIN_NAME,
    version: '0.1.0',
    description: 'Claude Code skills and slash commands converted by AgyObsidian for Antigravity CLI.',
    author: {
      name: 'AgyObsidian',
    },
    keywords: ['agy-obsidian', 'claude-code', 'skills', 'slash-commands', 'antigravity'],
  };
...
function buildSkillMarkdown(item: WritableTool): string {
  ...
  return [
    ...
    `# ${item.tool.name}`,
    '',
    `Converted by AgyObsidian from a local ${sourceLabel}.`,
    '',
...
function buildCommandMarkdown(item: WritableTool): string {
  ...
  return [
    ...
    `# /${item.tool.name}`,
    '',
    `Converted by AgyObsidian from \`${item.tool.sourcePath}\`.`,
    '',
    sourceBody || description,
    '',
  ].join('\n');
...
function buildReadme(items: WritableTool[]): string {
  const lines = [
    '# AgyObsidian Claude Tools',
    '',
    'Generated by AgyObsidian from local Claude Code skills and slash commands.',
    '',
