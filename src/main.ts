...
import type { AgyObsidianSettings } from './core/types';
import { DEFAULT_SETTINGS } from './core/types';
import { buildCopilotExtension } from './core/editor/CopilotExtension';
import { AgyObsidianView, VIEW_TYPE_AGY_OBSIDIAN } from './ui/AgyObsidianView';
...
import { AgyObsidianSettingsTab } from './ui/settings/AgyObsidianSettingsTab';

export const AGY_OBSIDIAN_ICON = 'bot';
...
export default class AgyObsidianPlugin extends Plugin {
  settings: AgyObsidianSettings;
  agent: AntigravityProvider;
  private lastActiveSupportedFile: TFile | null = null;
  ...
  async onload(): Promise<void> {
    ...
    this.registerEditorExtension(buildCopilotExtension(this));
    
    this.registerView(VIEW_TYPE_AGY_OBSIDIAN, (leaf) => new AgyObsidianView(leaf, this));

    this.addRibbonIcon(AGY_OBSIDIAN_ICON, 'Open AgyObsidian', () => {
      void this.activateView();
    });

    this.addCommand({
      id: 'open-agy-obsidian',
      name: 'Open AgyObsidian',
      callback: () => void this.activateView(),
    });

    this.addCommand({
      id: 'generate-image-from-note',
      name: 'Generate AgyObsidian image from active note',
    ...
    this.addSettingTab(new AgyObsidianSettingsTab(this));
  }

  onunload(): void {
    this.agent?.cancel();
    this.app.workspace.detachLeavesOfType(VIEW_TYPE_AGY_OBSIDIAN);
  }
  ...
  refreshAgyObsidianViews(): void {
    for (const leaf of this.app.workspace.getLeavesOfType(VIEW_TYPE_AGY_OBSIDIAN)) {
      const view = leaf.view;
      if (view instanceof AgyObsidianView) view.refreshDiscoveredSlashCommands();
    }
  }

  async activateView(): Promise<void> {
    const { workspace } = this.app;
    let leaf = workspace.getLeavesOfType(VIEW_TYPE_AGY_OBSIDIAN)[0];
    if (!leaf) {
      const rightLeaf = workspace.getRightLeaf(false);
      if (!rightLeaf) return;
      leaf = rightLeaf;
      await leaf.setViewState({ type: VIEW_TYPE_AGY_OBSIDIAN, active: true });
    }
    if (leaf) {
  ...
  async getActiveNoteContext(): Promise<ActiveNoteContext | null> {
    ...
    const file = this.getActiveSupportedFile();
  ...
  getActiveSupportedFile(): TFile | null {
    const markdownViewFile = this.app.workspace.getActiveViewOfType(MarkdownView)?.file;
    const activeFile = this.app.workspace.getActiveFile();
    const file = markdownViewFile || activeFile || this.lastActiveSupportedFile;

    if (file && (file.extension === 'md' || file.extension === 'canvas')) {
      this.lastActiveSupportedFile = file;
      return file;
    }

    return this.lastActiveSupportedFile;
  }
  ...
  async attachCurrentNoteToChat(): Promise<void> {
    const activeFile = this.getActiveSupportedFile();
    if (!activeFile) {
      new Notice('Open a markdown or canvas note before attaching it.');
      return;
    }
  ...
  refreshOpenViews(): void {
    for (const leaf of this.app.workspace.getLeavesOfType(VIEW_TYPE_AGY_OBSIDIAN)) {
      const view = leaf.view;
      if (view instanceof AgyObsidianView) {
  ...
  async generateImageFromActiveNote(): Promise<void> {
    ...
    if (!context || !activeFile) {
      new Notice('Open a markdown or canvas note before generating an AgyObsidian image.');
      return;
    }
    ...
    const progressModal = new VisualGenerationProgressModal(this.app, 'Generating AgyObsidian image');
    progressModal.open();
    progressModal.addStep(`AgyObsidian source note: ${activeFile.path}`);
    ...
    try {
      ...
      const draftedPrompt = await draftVisualPrompt({
        ...
        mode: input.mode,
        outputType: 'png',
        userPrompt: buildAgyObsidianDirection(input.prompt),
        noteContent,
      ...
      const generationProgressModal = new VisualGenerationProgressModal(this.app, 'Generating AgyObsidian image');
      ...
      generationProgressModal.finish(`AgyObsidian image embedded: ${generated.path}`, 'success');
      new Notice(`AgyObsidian image embedded: ${generated.path}`);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      activeProgressModal.finish(`Error: ${message}`, 'error');
      console.error('[AgyObsidian] Image generation failed:', error);
      new Notice(`AgyObsidian image generation failed: ${message}`);
    }
  }
  ...
  async generateGrokVideoFromActiveNote(direction = ''): Promise<void> {
    ...
    const context = await this.getActiveNoteContext();
    if (!context || !activeFile) {
      new Notice('Open a markdown or canvas note before generating a Grok video.');
      return;
    }
  ...
  async probeAntigravityCapabilities(): Promise<void> {
    ...
    try {
      for await (const event of this.agent.query({
        ...
        prompt: [
          'Probe native Antigravity media capabilities for AgyObsidian.',
...
function buildAgyObsidianDirection(userPrompt: string): string {
  return [
    userPrompt,
    '',
    'AgyObsidian direction:',
