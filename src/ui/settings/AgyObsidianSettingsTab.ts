...
export class AgyObsidianSettingsTab extends PluginSettingTab {
  plugin: AgyObsidianPlugin;
  private setupLogEl: HTMLElement | null = null;

  constructor(plugin: AgyObsidianPlugin) {
  ...
  display(): void {
    ...
    containerEl.empty();
    containerEl.createEl('h2', { text: 'AgyObsidian' });

    const antigravityCard = containerEl.createDiv({ cls: 'agy-obsidian-settings-card' });
    antigravityCard.createEl('h3', { text: 'Antigravity CLI' });
    antigravityCard.createEl('p', {
      text: 'AgyObsidian uses Antigravity CLI as the native agent runtime for note-derived image generation. Video and TTS remain capability-gated until Antigravity exposes native support.',
    });
    const detectedAntigravity = findAntigravityCli('', buildProcessEnv(this.plugin.settings.environmentVariables).PATH);
    antigravityCard.createEl('p', {
      cls: 'agy-obsidian-settings-hint',
    ...
    const setupCard = containerEl.createDiv({ cls: 'agy-obsidian-settings-card' });
    ...
    new Setting(antigravityCard)
      ...
      .setDesc('Stored as an AgyObsidian preference and passed into AGY prompts. AGY still owns the actual model switch.')
    ...
    const pluginCard = containerEl.createDiv({ cls: 'agy-obsidian-settings-card' });
    ...
    pluginCard.createEl('p', {
      cls: 'agy-obsidian-settings-hint',
      text: 'Conversion copies local markdown instructions only, writes an agy-obsidian-claude-tools AGY plugin, then asks AGY to validate, install, and enable it.',
    });

    const externalCliCard = containerEl.createDiv({ cls: 'agy-obsidian-settings-card' });
    externalCliCard.createEl('h3', { text: 'External CLI connectors' });
    externalCliCard.createEl('p', {
      text: 'AgyObsidian can detect optional local CLIs and later route slash-command tasks to them. Missing CLIs stay disabled instead of blocking the plugin.',
    });
    ...
    const envCard = containerEl.createDiv({ cls: 'agy-obsidian-settings-card' });
    ...
    const imageCard = containerEl.createDiv({ cls: 'agy-obsidian-settings-card' });
    ...
    new Setting(imageCard)
      ...
      .addText((text) => text
        ...
        .onChange(async (value) => {
          this.plugin.settings.mediaFolder = value.trim() || 'attachments/agy-obsidian';
          await this.plugin.saveSettings();
        }));
  }
  ...
  private async importPlugins(source: AntigravityPluginImportSource): Promise<void> {
    ...
    try {
      ...
      this.plugin.refreshAgyObsidianViews();
