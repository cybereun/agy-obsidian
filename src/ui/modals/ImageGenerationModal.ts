...
export class ImageGenerationModal extends Modal {
  ...
  onOpen(): void {
    ...
    contentEl.empty();
    contentEl.createEl('h2', { text: 'Generate AgyObsidian image' });
    contentEl.createEl('p', {
      text: 'AgyObsidian will analyze the current note, draft an image prompt, generate an image with Antigravity CLI, and embed it at the top.',
    });

    new Setting(contentEl)
      .setName('Output')
      .setDesc('AgyObsidian v1 uses Antigravity-native raster image generation only.')
