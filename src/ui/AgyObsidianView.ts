...
import AgyObsidianPlugin, { AGY_OBSIDIAN_ICON } from '../main';
...
export const VIEW_TYPE_AGY_OBSIDIAN = 'agy-obsidian-view';
...
export class AgyObsidianView extends ItemView {
  private plugin: AgyObsidianPlugin;
  ...
  constructor(leaf: WorkspaceLeaf, plugin: AgyObsidianPlugin) {
    super(leaf);
    this.plugin = plugin;
  }

  getViewType(): string {
    return VIEW_TYPE_AGY_OBSIDIAN;
  }
  ...
  private buildHeader(header: HTMLElement): void {
    ...
    setIcon(logoEl, AGY_OBSIDIAN_ICON);
    titleContainer.createEl('h4', { text: 'AgyObsidian' });
    ...
    visualBtn.setAttribute('aria-label', 'Generate AgyObsidian image from note');
  ...
  private renderFileChips(): void {
    ...
    const activeFile = this.plugin.getActiveSupportedFile();
  ...
  private createFileChip(filePath: string, options: { current?: boolean; pinned?: boolean }): void {
    ...
    setIcon(remove, 'x');
    remove.setAttribute('aria-label', 'Remove note from AgyObsidian context');
  ...
  private async openRenderedLink(rawTarget: string, label: string): Promise<boolean> {
    ...
    const activePath = this.plugin.getActiveSupportedFile()?.path || '';
    ...
    if (target.endsWith('.md') || target.includes('/')) {
      new Notice(`AgyObsidian could not find note: ${target}`);
      return true;
    }
  ...
  private reopen(): void {
    ...
    void leaf.setViewState({ type: VIEW_TYPE_AGY_OBSIDIAN, active: true });
  }
  ...
  private async handleLocalSlashCommand(prompt: string): Promise<boolean> {
    if (prompt === '/skills') {
      ...
      const lines = [
        `Found ${BUILTIN_SKILLS.length} built-in AgyObsidian skills and ${tools.length} local Claude tools.`,
        '',
        '## Built-in AgyObsidian skills',
        ...
        '',
        'Built-in skills run directly through AgyObsidian with active-note context. Claude tools are shown in the slash picker when discovered locally.',
      ];
  ...
  private parseExternalSlashCommand(prompt: string): { ids: ExternalCliId[]; request: string } | null {
    ...
    if (!request) {
      return {
        ...
        request: 'Summarize what you can do from AgyObsidian in one concise paragraph.',
      };
    }
    return {
      ids: command === 'collab' ? ['claude', 'codex', 'grok'] : [command as ExternalCliId],
